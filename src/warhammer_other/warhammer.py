...
class Dice(Buff):
    ...
    def chances(self):
        return self.amount * [
            {i: Fraction(1, self.sides) for i in range(1, self.sides + 1)}
        ]
...
def phase(*buffs):
    ...
    buffs = [
        b for bs in buffs_.values() for b in (bs if isinstance(bs, list) else [bs])
    ]
    totals = collections.defaultdict(lambda: Fraction(0, 1))
    for reroll_chance, rerolls in (
        (Fraction(1, 1), reroll.values),
        (Fraction(reroll.value, 6), ()),
    ):
        for i in range(1, 7):
            raw = i
            if i in rerolls:
...
def damage(*buffs):
    buffs_ = read_buffs(buffs)
    damage = {buffs_.pop(Damage, Damage(0)).value: Fraction(1, 1)}
...
def binomial(n, k):
    a, b = sorted((k, n - k))
    return int(
        functools.reduce(operator.mul, range(b + 1, n + 1), 1)
        / functools.reduce(operator.mul, range(1, a + 1), 1)
    )
...
def plot(value):
    ...
    df = pd.DataFrame(
        [float(i) for i in data], columns=["Score"], index=list(range(1, top + 1))
    )
...
if __name__ == "__main__":
    ...
    print(
        "Plasma save",
        attack_(phase(Score(3), SelfWound(1)), phase(Score(3)), phase(Score(3))),
    )
    print(
        "Plasma damage",
        attack_(
            phase(Score(3), SelfWound(1)),
            phase(Score(3)),
            phase(Score(3)),
            damage(Damage(2)),
        ),
